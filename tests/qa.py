...
CHECKS = [
    ...
    ("COG · miembro fuera de su familia", f"SELECT count(*) FROM cognate_member cm JOIN cognate_set cs ON cs.id=cm.cognate_set_id JOIN form f ON f.id=cm.form_id WHERE cs.source='kaikki-etymology' AND cs.family IS NOT NULL AND NOT EXISTS (SELECT 1 FROM (VALUES {_FM_VALUES}) v(fam,member) WHERE v.fam=cs.family AND v.member=f.lect_id)", "fail", "cognados POR-ETYMON: el miembro debe ser un lect declarado de su familia (la red cog cruza familias por diseño)"),
    ("COG · miembro cog con lect no declarado", f"SELECT count(*) FROM cognate_member cm JOIN cognate_set cs ON cs.id=cm.cognate_set_id JOIN form f ON f.id=cm.form_id WHERE cs.source='kaikki-cog' AND f.lect_id <> ALL({_ALL_MEMBERS_SQL})", "fail", "red cog (cross-familia): lect debe existir en alguna familia declarada"),
